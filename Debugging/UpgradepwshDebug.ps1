...
try{
    ...
    If( $PreRelease.IsPresent ){
        $ReleaseToDownload = $ReleaseMetadata.NextRelesaeTag
    }
    else{
        $ReleaseToDownload = $ReleaseMetadata.RelesaeTag
    }

    $Releases = Invoke-RestMethod -Uri $ReleaseUrl
    $LatestRelease = ($Releases | Where-Object -FilterScript { $_.Tag_Name -eq $ReleaseToDownload }).assets | Select-Object -Property Name,Browser_Download_Url -ErrorAction Stop
...
# Finally{
#     Remove-Item -Path $env:Temp\UpdatePwsh.bat -Force -ErrorAction SilentlyContinue
# }
